...
void task_data_acquisition(void *p_parameter)
{
	...
	st_data_sensors data_sensors={.ambient_humidity=20,.ambient_temperature=20,.batery=30,.radiacion=10,.soil_moisture_1=20,.soil_moisture_2=0,.latitude=-21.5537892,.longitude=-64.747228};
	...
	while (1)
	{
		...
		switch (event_data_adquisition.states_data_adquisition)
		{
			case ADQUISITION:
				...
				HAL_ADC_Stop(&hadc1);
				config_alarms.sensors_data=data_sensors;
				xQueueSend(queue_data,&data_sensors,0);
				config_alarms.event_alarms=MONITOR;
				xQueueSend(queue_alarms,&config_alarms,0);
				break;
			default:
				break;
		}
